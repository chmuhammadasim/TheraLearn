      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {queries.map((query) => (
          <motion.div
            ...
            className="bg-white shadow-2xl rounded-xl p-6 hover:shadow-3xl transition-shadow duration-300 relative transform"
          >
      ...
      {selectedQuery && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
          <motion.div
            ...
            className="bg-white rounded-lg p-10 shadow-lg w-full max-w-lg"
          >
            <h2 className="text-3xl font-bold mb-6 text-center text-purple-700">Reply to {selectedQuery.name}</h2>
            <div className="mb-5">
              <input
                type="text"
                name="subject"
                ...
                className="w-full border border-gray-300 p-3 rounded-lg mb-4 focus:ring-2 focus:ring-purple-500"
              />
              <textarea
                name="replyMessage"
                ...
                onChange={handleReplyChange}
                className="w-full border border-gray-300 p-3 rounded-lg h-36 focus:ring-2 focus:ring-purple-500"
              />
            </div>
            <div className="flex justify-between">
              <button
                onClick={() => handleSendReply(selectedQuery.email)}
                className="bg-gradient-to-r from-green-400 to-teal-500 text-white px-6 py-2 rounded-full shadow-md hover:bg-teal-600 transition-all duration-300"
              >
                Send Reply
              </button>
              <button
