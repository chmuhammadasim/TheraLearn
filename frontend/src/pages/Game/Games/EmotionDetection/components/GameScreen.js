...
const GameScreen = ({ player, onGameOver, onUpdatePlayer }) => {
  const [currentEmotion, setCurrentEmotion] = useState('');
  const [score, setScore] = useState(player.score || 0);
  const [level, setLevel] = useState(player.level || 1);
  const [attempts, setAttempts] = useState(player.attempts || 0);
  ...
  useEffect(() => {
    const saveProgress = async () => {
      try {
        await axios.put(`/api/players/${player._id}`, {
          score,
          level,
          attempts,
          timePlayed: LEVEL_DURATION - timeLeft
        });
      } catch (err) {
  ...
  }, [score, level, attempts, timeLeft, player._id]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-purple-50 to-pink-50 p-2 sm:p-4 md:p-6">
      <motion.div 
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        ...
        <div className="flex flex-col lg:flex-row gap-4 md:gap-6 lg:gap-8">
          ...
          <motion.div 
            layout
            className={`bg-white rounded-xl shadow-lg p-4 md:p-6 w-full ${isFullscreen ? 'lg:hidden' : 'lg:w-1/3'} border border-indigo-100 transition-all duration-300 ease-in-out backdrop-blur-sm bg-white/90`}>
            <motion.h2 
              layout
              className="text-xl md:text-2xl font-bold text-indigo-700 mb-3 md:mb-5 flex items-center">
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-4 mb-4 md:mb-6">
              <motion.div 
                ...
                className="bg-indigo-50 rounded-lg p-2 md:p-3 shadow-sm transition-all duration-200 hover:shadow-md">
                <h3 className="text-xs md:text-sm font-semibold text-indigo-600 uppercase tracking-wide">Player</h3>
                <p className="text-lg md:text-xl font-bold truncate">{player.name}</p>
              </motion.div>
              
              <motion.div 
                whileHover={{ scale: 1.03 }}
                className="bg-purple-50 rounded-lg p-2 md:p-3 shadow-sm transition-all duration-200 hover:shadow-md">
              ...
              <motion.div 
                whileHover={{ scale: 1.03 }}
                className="bg-blue-50 rounded-lg p-2 md:p-3 shadow-sm transition-all duration-200 hover:shadow-md">
              ...
              <motion.div 
                whileHover={{ scale: 1.03 }}
                className="bg-teal-50 rounded-lg p-2 md:p-3 shadow-sm transition-all duration-200 hover:shadow-md">
              ...
              <div className="w-full bg-gray-200 rounded-full h-2 md:h-2.5 overflow-hidden">
                <motion.div 
                  ...
                  className={`h-2 md:h-2.5 rounded-full ${
                    ...
                    timeLeft < LEVEL_DURATION/2 ? 'bg-amber-500' : 'bg-indigo-600'
                  }`}
                ></motion.div>
              </div>
            </div>
            ...
            <motion.div 
              ...
              className="mt-4 md:mt-6 p-3 md:p-5 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-lg shadow-md text-white group">
              ...
              <p className="text-2xl md:text-3xl font-bold text-center capitalize">
                {currentEmotion}
                {showSuccess && (
                  <motion.span 
                    initial={{ scale: 0 }}
          ...
          <motion.div 
            ...
            <motion.div 
              layout
              className="bg-white rounded-xl shadow-lg p-4 md:p-6 border border-indigo-100 backdrop-blur-sm bg-white/90">
              <div className="flex justify-between items-center mb-3">
                <motion.h2 
                  layout
                  className="text-lg md:text-xl font-bold text-indigo-700">Make this face:</motion.h2>
                <motion.button 
              ...
              <motion.div 
                ...
                className="flex flex-col md:flex-row justify-center items-center p-4 md:p-6 bg-gradient-to-r from-indigo-50 via-purple-50 to-indigo-100 rounded-lg shadow-sm">
                <AnimatePresence mode="wait">
                  <motion.div
                    key={currentEmotion}
                ...
                <AnimatePresence>
                  {showSuccess && (
                    <motion.div 
                      ...
                      exit={{ opacity: 0, y: -20 }}
                      className="bg-gradient-to-r from-green-400 to-emerald-500 text-white text-base md:text-xl p-2 md:p-4 ml-0 md:ml-8 mt-4 md:mt-0 rounded-lg shadow-lg"
                    >
            ...
            <motion.div 
              layout
              className="relative rounded-xl overflow-hidden shadow-2xl border-4 border-white group"
            >
              <video 
                ref={videoRef} 
                autoPlay 
                ...
                style={{ height: isFullscreen ? '550px' : '450px', objectFit: 'cover' }}
              />
              <canvas 
                ref={canvasRef} 
                width="640" 
              ...
              <motion.div 
                className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-sm text-white px-3 py-1 rounded-full text-xs md:text-sm"
              ...
              {/* Level badge */}
              <div className="absolute top-4 left-4 bg-indigo-600/80 backdrop-blur-sm text-white px-3 py-1 rounded-full text-xs md:text-sm font-bold">
                Level {level}
              </div>
            ...
            <motion.div 
              ...
              className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-xl p-3 md:p-4 text-center shadow-md text-white"
            >
              <p className="text-sm md:text-base font-medium">
                Express <span className="font-bold">{currentEmotion}</span> to earn points! {timeLeft < 3 && "Hurry up!"}
              </p>
            </motion.div>
          </motion.div>
        </div>
